import { Client, Databases, Query, ID, Users, TablesDB } from 'node-appwrite';
// ============================================================================
// CONSTANTS
// ============================================================================
...
const REFERRALS_TABLE_ID = 'referrals';
...
// ============================================================================
// REFERRAL CLAIM (Tables DB + user_profiles documents)
// ============================================================================
/**
 * Normalize function request body (Appwrite may pass string or object).
 */
...
/**
 * Read header case-insensitively (e.g. x-appwrite-user-id).
 */
...
async function getSettingValue(tablesDB, key, fallback) {
...
function isRowAlreadyExistsError(err) {
  const code = err?.code;
...
/**
 * POST /claim-referral — requires x-appwrite-user-id (session JWT execution).
 * Body: { referralCode: string }
 */
async function claimReferral(databases, tablesDB, users, req, log) {
  const userId = getRequestHeader(req, 'x-appwrite-user-id');
  ...
  const codeRaw = body.referralCode;
  if (
    codeRaw == null ||
    ...
    return {
      status: 400,
      body: { success: false, error: 'referralCode is required' },
    };
  }
  ...
  if (!/^[A-Z0-9]{4,32}$/.test(code)) {
    return {
      status: 400,
      body: { success: false, error: 'Invalid referral code format' },
    };
  }
  let authUser;
  try {
    ...
    return { status: 401, body: { success: false, error: 'Invalid user' } };
  }
  if (!authUser.emailVerification) {
    return {
      ...
      body: {
        success: false,
        error: 'Email must be verified before claiming a referral',
      },
    };
  }
  const existingReferral = await tablesDB.listRows({
    databaseId: DATABASE_ID,
    tableId: REFERRALS_TABLE_ID,
    queries: [Query.equal('refereeAuthID', userId), Query.limit(1)],
  });
  if (existingReferral.rows && existingReferral.rows.length > 0) {
    return {
      status: 200,
      body: { success: true, alreadyClaimed: true },
    };
  }
  const refereeRes = await databases.listDocuments(
    DATABASE_ID,
    USER_PROFILES_TABLE_ID,
    [Query.equal('authID', userId), Query.limit(1)]
  );
  if (refereeRes.total === 0) {
    return {
      status: 404,
      body: { success: false, error: 'User profile not found' },
    };
  }
  const refereeProfileDoc = refereeRes.documents[0];
  let referrerRes = await databases.listDocuments(
    DATABASE_ID,
    USER_PROFILES_TABLE_ID,
    [Query.equal('referralCode', code), Query.limit(2)]
  );
  if (referrerRes.total === 0) {
    referrerRes = await databases.listDocuments(
      DATABASE_ID,
      USER_PROFILES_TABLE_ID,
      [Query.equal('referalCode', code), Query.limit(2)]
    );
  }
  if (referrerRes.total === 0) {
    return {
      status: 404,
      body: { success: false, error: 'Invalid referral code' },
    };
  }
  const referrerProfileDoc = referrerRes.documents[0];
  if (referrerProfileDoc.authID === userId) {
    return {
      status: 400,
      body: { success: false, error: 'Cannot use your own referral code' },
    };
  }
  if (referrerProfileDoc.isBlocked === true) {
    return {
      status: 400,
      body: { success: false, error: 'Referral code is not valid' },
    };
  }
  const referrerPtsStr = await getSettingValue(
    tablesDB,
    'referral_points_referrer',
    '100'
  );
  const refereePtsStr = await getSettingValue(
    tablesDB,
    'referral_points_referee',
    '100'
  );
  const referrerPts = parsePositiveIntSetting(referrerPtsStr, 100);
  const refereePts = parsePositiveIntSetting(refereePtsStr, 100);
  const completedAt = new Date().toISOString();
  const rowData = {
    refereeAuthID: userId,
    referralCode: code,
    referrerPointsAwarded: referrerPts,
    refereePointsAwarded: refereePts,
    status: 'completed',
    completedAt,
    refereeProfileId: refereeProfileDoc.$id,
    referrerProfile: referrerProfileDoc.$id,
  };
  try {
    ...
    if (isRowAlreadyExistsError(err)) {
      log('claim-referral: duplicate row (race), treating as idempotent');
      return {
        status: 200,
        body: { success: true, alreadyClaimed: true },
      };
    ...
    log(`claim-referral: createRow failed: ${msg}`);
    throw err;
  }
  const refereeTotal = (refereeProfileDoc.totalPoints ?? 0) + refereePts;
  const referrerTotal = (referrerProfileDoc.totalPoints ?? 0) + referrerPts;
  try {
    await databases.updateDocument(
      DATABASE_ID,
      USER_PROFILES_TABLE_ID,
      refereeProfileDoc.$id,
      {
        totalPoints: refereeTotal,
      }
    );
    await databases.updateDocument(
      DATABASE_ID,
      USER_PROFILES_TABLE_ID,
      referrerProfileDoc.$id,
      {
        totalPoints: referrerTotal,
      }
    );
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    log(
      `claim-referral: points update failed after referral row created: ${msg}`
    );
    return {
      status: 500,
      body: {
        success: false,
        error:
          'Referral recorded but points update failed. Please contact support.',
      },
    };
  }
  return {
...
export default async function handler({ req, res, log, error }) {
  try {
    ...
    // REFERRAL CLAIM
    // ========================================================================
    if (req.path === '/claim-referral' && req.method === 'POST') {
      log('Processing claim-referral request');
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to claim referral';
        error(`claim-referral error: ${errorMessage}`);
        return res.json(
          {
            success: false,
            error: errorMessage,
    ...
    return res.json({
      success: false,
      error:
        'Invalid endpoint. Available endpoints: POST /get-events-by-location, POST /get-active-trivia, POST /submit-answer, POST /dismiss-trivia, POST /create-user, POST /delete-account, POST /update-user-status, POST /get-user-by-email, POST /reset-password-after-otp, POST /claim-referral, GET /ping',
    });
  } catch (err) {
    const errorMessage =
